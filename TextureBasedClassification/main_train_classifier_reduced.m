...
cnst.FeatureDataSource = './datasets/HISTO2/UID_60007/'; % location with ONLY feature sets
cnst.ReducedFeatures=true; % true if already ran feature selection, false if no feature selection
...
countr = 1096;             % do not change. Table indices for summary.

%% BODY
% iterate through all methods and classifiers, perform all experiments
%firstdirectory=cd(cnst.FeatureDataSource);
reducedFeatureSets=dir([cnst.FeatureDataSource,'/*labhist*.mat']);
%reducedFeatureSets=dir(cnst.FeatureDataSource)
cnst.allFsets=string({reducedFeatureSets.name});
%cnst.allFsets=cnst.allFsets(countr:end);
%cd(firstdirectory)

for FSetCount = cnst.allFsets
    
  featureID = 1096; % for summary grid
  
% prepare variables for display and show status
[currFSet,currClassifier, currSelecter] = findNamesReduced(FSetCount); 
disp([cnst.crossbar,10,'starting iteration with ',currFSet,currSelecter]); countdown(5);

% if cnst.ReducedFeatures==true
    [CatNames, source_and_target, myData, myLabels, myLabelCats, numfeat] = ...
    load_dataset_Automatic(FSetCount, cnst.FeatureDataSource); %
% else
...
for currNumClass = [numel(maxClasses)] % first and last element [1,numel(maxClasses)]
    ...
    currClassifierName = [randID, '-row_',num2str(countr),...
      '-MaxClass_',num2str(maxClass),'-Data_',cnst.DataFolder,...
      '-Feat_',currFSet,'_',num2str(numfeat),...
      '-select_',currSelecter,'-Classif_',currClassifier];
    ...
    if contains(currClassifier,'optnn')>0
        save([cnst.saveDir 'OptNNFitParams' currClassifierName '.mat'], 'BayesoptResults');
    end
	% display status
...
if cnst.showResults
 ...
 % end iteration through classifier (classifier methods)
...
writetable(struct2table(summaryTable),[cnst.saveDir,randID,'_summary_72220','.csv']);

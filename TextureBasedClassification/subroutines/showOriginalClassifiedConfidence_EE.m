...
function showOriginalClassifiedConfidence_EE(imgResult,cnst,plotName,curr_impath,CatNames)
    ...
    foundClasses(foundClasses == 9) = []; % remove 0 class
    foundClasses(foundClasses == 0) = [];
    count = 1;
    ...
    plot1 = 0; plot2 = 1; plot3 = 1; plot4 = 1; plot5 = 1; %plot6 = 1;
...
%     suptitle(plotName); drawnow;
    ...
    [myCmap,CatNamesShort] = tissueColorMap_lmb(cnst.maxClass);
    ...
    imshow(highestClass,myCmap);
    c=colorbar;
    c.Ticks=[1,2,3,4,5,6];
    c.TickLabels=CatNamesShort;
...
%     subplot(1,2,2), imshow(highestClassRGBreconstr)
    ...
    for i=1:cnst.maxClass
        subplot(3,round((cnst.maxClass+1)/3,0),i)
        imshow(imgResult(:,:,i),[]);
        colormap(gca, parula)

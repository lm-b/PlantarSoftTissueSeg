...
function out = tileClassify_fractal_cluster(imgIn,myClassifier,...
    numFeats,numClasses,featType,cnst,setStats, GaborArray, idx, lawsFilters,numblock)

    % compute features, then apply classifier to features
    featureVector = computeFeatureVector_tileClassify(imgIn,featType,GaborArray, idx, lawsFilters, numblock);
    
    % Apply Z-score normalization
    if contains(["best2", "best_all"], featType)
        featureVector=featureVector-mean(myClassifier.X(:))./std(myClassifier.X(:));
    end
